  ...
  return (
    <>
      <button
        ...
        className={` ${selectedStatus ? 'bg-primary  text-white' : 'bg-secondary hover:bg-secondary/80 hover:text-white transition duration-300'} rounded-md 
...

      {
              ...
              <div className="absolute p-2  translate-x-4 max-w-96 -translate-y-20 top-auto left-0 bottom-0 right-auto   rounded-md bg-opacity-secondary  text-color-text flex flex-col cursor-pointer items-center justify-center ">
                <div ref={AnimeStatusRef} className=" flex flex-col py-2 w-full bg-opacity-secondary ">
                  ...
                  {DropDownButtonText.map((text, index) => <DropDownButton clickHandler={ClickStatusHandler} key={index} text={text} />)}
