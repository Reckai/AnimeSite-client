...
import {Avatar, AvatarImage} from "@/app/Components/Header/_components/Avatar/AvaterComponent";

function DropDownMenu({ img }: { img: string | null | undefined }) {
  const router = useRouter();

  const session = useSession();
  const USERLINK = `/user/${session.data?.user?.id || ''}`
