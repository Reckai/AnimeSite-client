...
export const useStatus = (initialAnimeStatus?: AnimeStatus) => {
	const initialStatusName = React.useMemo(() => {
		return (
			DropDownButtonProperty.find((property) => property.statusProperty === initialAnimeStatus)
				?.name || DropDownButtonInitialText
		);
	}, [initialAnimeStatus]);

	const [status, setStatus] = React.useState<string>(
		initialStatusName || DropDownButtonInitialText
	);

	const changeStatus = (animeStatus?: AnimeStatus) => {
		const newStatusName =
			DropDownButtonProperty.find((property) => property.statusProperty === animeStatus)?.name ||
			DropDownButtonInitialText;
